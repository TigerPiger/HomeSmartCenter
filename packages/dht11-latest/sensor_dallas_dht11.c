...
static uint8_t dht11_check(rt_base_t pin)
{
    ...
    if(retry >= 100)
    {
        LOG_E("CONNECT_FAILED, retry %d times", retry);
...
static rt_size_t dht11_polling_get_data(rt_sensor_t sensor, struct rt_sensor_data *data)
{
    ...
    temperature_humidity = dht11_get_temperature((rt_base_t)sensor->config.intf.arg);
...
int rt_hw_dht11_init(const char *name, struct rt_sensor_config *cfg)
{
    ...
    if (!dht11_init((rt_base_t)cfg->intf.arg))
    {
        ...
        sensor->info.name       = "DHT11";
        sensor->info.unit       = RT_SENSOR_UNIT_CELSIUS;
        sensor->info.intf_type  = RT_SENSOR_INTF_ONEWIRE;
        sensor->info.scale.range_max  = SENSOR_TEMP_RANGE_MAX;
        sensor->info.scale.range_min  = SENSOR_TEMP_RANGE_MIN;
        sensor->info.acquire_min = 100; /* Read ten times in 1 second */
